// Rediger denne fila for å endre tekstene på siden.
window.GYM_CONFIG = {
  subtitle: 'Forskernatt · NTNU',
  title: 'Hva er gym for deg?',
  questions: [
    'Beskriv "gym" med ett eller to ord.',
    'Nevn én ting du savner i gym.',
  ],
  thanks: 'Takk! Se etter ordene dine ↑',
  // Farger på ordene (linjefargene i en gymsal)
  palette: ['#ffd23f', '#ff5a5f', '#4da3ff', '#f4f1ea', '#3ddc84', '#ff9f43'],
  // Ordskyene er skjult bak en «skillevegg» og vises så lenge etter at noen har svart (ms)
  revealMs: 10000,
  curtainText: 'Svar først, så får du se hva andre har svart',
  curtainTextEmpty: 'Ingen svar ennå. Bli den første!',
  // Går tilbake til spørsmål 1 hvis ingen skriver på spørsmål 2 på så lenge (ms)
  idleResetMs: 60000,
};
